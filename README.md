...
- **分类一键加书签**：侧栏每个分类行右侧有「＋」，点击后新建书签已自动选好该分类，保存即归入其中并排在最前。
...
| v4.3 | 2026-09-25 | 侧栏分类行新增「＋」按钮：点击直接新建书签并归入该分类，保存后排在该分类最前（自动切换并聚焦到该分类）；移动端胶囊分类同样支持 |
| v4.2 | 2026-09-20 | 移动端布局重做：头部换行（标题+按钮 / 分区标签 / 搜索各一行）、分类变顶部横向滚动胶囊条、书签两列撑满；整体体验优化：首屏骨架屏、图标懒加载防抖、搜索清除按钮、Esc 关闭浮层、键盘焦点可见、深色滚动条 + `color-scheme:dark` 等；「分区管理」支持拖动排序（新增分区 `sort` 字段） |
